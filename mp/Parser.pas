...
uses
  ...
  ParseMethod, ParseTypes, TextConsts, TextBuilder, ValueTypes;
...
type
  ...
  protected
    ...
    procedure Check(const Text: string); overload; virtual;
    procedure Check(var Syntax: TSyntax; const Kind: TOperatorKind; const Text: string; const ScriptArray: TScriptArray;
      const Handle: Integer); overload; virtual;
    procedure Check(const ItemArray: TTextItemArray; const Index: Integer; const Data: TItemData; const ScriptArray: TScriptArray;
      const Parameter: Boolean = False); overload; virtual;
    function InternalCompile(const Text: string; var ScriptArray: TScriptArray; const Parameter: Boolean; var Idle: PFunction): TScript; virtual; abstract;
  ...
  public
    ...
    procedure StringToScript(const Text: string; out Script: TScript); virtual; abstract;
    ...
    function ScriptToString(const AScript: TScript; const TypeMode: TRetrieveMode = rmUser): string; overload; virtual; abstract;
    ...
    function DefaultFunction(const AFunction: PFunction; out Value: TValue; const LValue, RValue: TValue;
      const ParameterArray: TParameterArray): Boolean; virtual;
    procedure Prepare; virtual;
...
type
  ...
  private
    ...
    FShrHandle: Integer;
  protected
    function ParseMethod(var Text: string; const StartIndex, EndIndex: Integer;
    ...
    function DecompileMethod(var Index: Integer; const Header: PScriptHeader;
      const ItemHeader: PItemHeader; const Item: PScriptItem;
      const Data: Pointer): Boolean; virtual;
    function Order(const Coverage: TPriorityCoverage; var ItemArray: TTextItemArray; var ScriptArray: TScriptArray; var Idle: PFunction): Boolean; virtual;
    function Morph(var AItemArray: TTextItemArray; out MItemArray: TTextItemMultiArray): Boolean;
    procedure Parse(var Text: string; var ScriptArray: TScriptArray; var Idle: PFunction); virtual;
    function InternalCompile(const Text: string; var ScriptArray: TScriptArray; const Parameter: Boolean; var Idle: PFunction): TScript; override;
    function ExecuteFunction(var Index: Integer; const ItemHeader: PItemHeader;
      const LValue: TValue; const Fake: Boolean = False): TValue; override;
    procedure ExecuteInternalScript(Index: Integer); virtual;
    procedure ReadParameterArray(var Index: Integer; out ParameterArray: TParameterArray; const ParameterType: TParameterType = ptParameter;
      const Fake: Boolean = False); override;
    procedure InternalOptimize(const Index: Integer; out AScript: TScript); override;
  ...
  public
    ...
    procedure StringToScript(const Text: string; out AScript: TScript); overload; override;
    ...
    function ScriptToString(const AScript: TScript; const TypeMode: TRetrieveMode = rmUser): string; override;
    function ScriptToString(const AScript: TScript; const Delimiter: string;
...
function MakeBracketData(const AScriptArray: PScriptArray; const AParameter: Boolean; const AIdle: PFunction): TBracketData;
...
uses
  ...
  ParseCommon, ParseErrors, ParseManager, ParseMessages, ParseUtils, ParseValidator, {$IFDEF DELPHI_XE3}System.UITypes, {$ENDIF}
...
begin
  ...
  begin
    Notify(ntBTA, Self);
    Validator.Check(AType.Name, rtName);
...
procedure TCustomParser.Check(const Text: string);
var
          ...
          if not CharInSet(Text[J], Blanks) then
            if Text[J] = FBracket[btLeft] then
              if not AFunction.Method.Parameter.RParameter and (AFunction.Method.Parameter.Count = 0) then
                raise Error(RTextExcessError, [AFunction.Name])
              else
                Break
            else
              if AFunction.Method.Parameter.Count > 0 then
                raise Error(AParameterExpectError, [AFunction.Name])
              else
                Break;
  finally
    FlagArray := nil;
...
{$WARNINGS OFF}
procedure TCustomParser.Check(const ItemArray: TTextItemArray; const Index: Integer; const Data: TItemData; const ScriptArray: TScriptArray;
...
begin
  if GetType(ItemArray[Index].THandle, AType) then
    case Data.Code of
      icNumber:
        begin
          if AType.Handle^ = FStringHandle then raise Error(StringTypeError, [ValueToText(Data.Number^)]);
          if Index > Low(ItemArray) then
            raise Error(NumberTypeError, [AType.Name, ValueToText(Data.Number^),
              RestoreText(Self, ItemArray, ScriptArray, Parameter)]);
        end;
      icFunction:
        begin
          ...
          begin
            if AFlag then raise Error(StringTypeError, [AFunction.Name]);
            if BFlag then raise Error(FunctionTypeError, [AType.Name, AFunction.Name, RestoreText(Self, ItemArray, ScriptArray, Parameter)]);
          end;
        end;
      icScript:
        begin
          ...
          if (AFlag or BFlag) and GetFunction(Data.Handle^, AFunction) then
          begin
            if AFlag then raise Error(StringTypeError, [ScriptToString(Data.Script^, rmUser)]);
            if BFlag then raise Error(ScriptTypeError, [AType.Name, Embrace(ScriptToString(Data.Script^, rmUser), FBracket), RestoreText(Self, ItemArray, ScriptArray, Parameter)]);
          end;
        end;
      icParameter:
        ...
        begin
          ...
          try
            raise Error(StringTypeError, [ScriptToString(Script, rmUser)]);
          finally
            Script := nil;
          end;
        end;
...
procedure TCustomParser.Check(var Syntax: TSyntax; const Kind: TOperatorKind; const Text: string;
  const ScriptArray: TScriptArray; const Handle: Integer);
var
  ...
  case Kind of
    okLatest:
      case Syntax.PriorKind of
        okFunction:
          if Assigned(PriorFunction) then
          begin
            if Assigned(PriorFunction) and (PriorParameter.Count > 0) then
              raise Error(RestoreText(Self, Text, ScriptArray), AParameterExpectError, [PriorFunction.Name]);
            if Assigned(PriorFunction) and PriorParameter.RParameter then
              raise Error(RestoreText(Self, Text, ScriptArray), RTextExpectError, [PriorFunction.Name]);
          end;
      end;
    okNumber, okScript:
      case Syntax.PriorKind of
        okNumber, okScript, okParameter: raise Error(FunctionExpectError, [RestoreText(Self, Text, ScriptArray)]);
        okFunction:
          if Assigned(PriorFunction) and Assigned(PriorParameter) and ((PriorParameter.Count > 0) or not PriorParameter.RParameter) then
            raise Error(RestoreText(Self, Text, ScriptArray), RTextExcessError, [PriorFunction.Name]);
      end;
    okFunction:
      begin
        case Syntax.PriorKind of
          okNewest:
            if Assigned(PriorParameter) and PriorParameter.LParameter then raise Error(LTextExpectError, [RestoreText(Self, Text, ScriptArray)]);
          okNumber, okScript, okParameter:
            if Assigned(AFunction) and Assigned(Parameter) and ((Parameter.Count > 0) or not Parameter.LParameter) then
              raise Error(RestoreText(Self, Text, ScriptArray), LTextExcessError, [AFunction.Name]);
          okFunction:
            ...
            begin
              if PriorParameter.RParameter and Parameter.LParameter then
                raise Error(RestoreText(Self, Text, ScriptArray), AMutualExcessError, [PriorFunction.Name, AFunction.Name]);
              if not PriorParameter.RParameter and not Parameter.LParameter then
                raise Error(RestoreText(Self, Text, ScriptArray), BMutualExcessError, [PriorFunction.Name, AFunction.Name]);
            end;
        end;
    ...
    okParameter:
      case Syntax.PriorKind of
        okNewest, okNumber, okScript, okParameter: raise Error(FunctionExpectError, [RestoreText(Self, Text, ScriptArray)]);
        okFunction:
          if Assigned(PriorFunction) and (PriorParameter.Count = 0) then
            raise Error(RestoreText(Self, Text, ScriptArray), AParameterExcessError, [PriorFunction.Name]);
      end;
  end;
  ...
  case Item.Code of
    ...
    StringCode:
      begin
        ...
        AData.ItemText.Append(Pointer(Index + SizeOf(TCode) + SizeOf(TScriptString)), Item.ScriptString.Size div SizeOf(Char));
...
begin
  ...
  try
    ...
    begin
      ...
      ReadParameterArray(Index, ParameterArray, TParameterType(Parameter.Kind = pkReference));
      RValue := EmptyValue;
    end
    else begin
      RValue := EmptyValue;
      ...
      case AFunction.Kind of
        ...
        fkMethod:
          case AFunction.Method.MethodType of
            ...
            mtParameterArray: Result := AFunction.Method.DMethod(AFunction, AType, ParameterArray);
            mtVariable:
...
begin
  ...
  begin
    Notify(ntBFA, Self);
    if AFunction.Handle^ <> FInternalHandle then Validator.Check(AFunction.Name, rtName);
...
function TParser.InternalCompile(const Text: string; var ScriptArray: TScriptArray; const Parameter: Boolean; var Idle: PFunction): TScript;
var
  AText, S: string;
  ...
  begin
    case Code of
      ...
      icScript, icParameter:
        begin
          if GetType(AItem.THandle, AType) then WriteType(Code, AType.ValueType);
          WriteScript(Result, AScript, Code = icParameter, @ItemIndex);
        end;
    end;
  end;
...
begin
  Result := nil;
  AText := Text;
  if AText = '' then raise Error(EmptyTextError);
  ...
  begin
    ...
    try
      ...
      try
        if CFlag then Order(pcTotal, AItemArray, ScriptArray, Idle);
        ...
        begin
          ...
          try
            ...
            begin
              ...
              try
                if Sign then
                  AScript := BCache.Find(TOperatorArray[toNegative] + AItem.Text)
                else
                  AScript := BCache.Find(AItem.Text);
              finally
                Leave(BCache.Lock^);
              end;
            end
            ...
            if Assigned(AScript) then Add(Result, AScript)
              ...
              if Data.TextType = ttNumber then Write(icNumber)
                ...
                if AItem.Text = '' then raise Error(TextError, [AText]);
                      ...
                      begin
                        try
                          ...
                          begin
                            Enter(ACache.Lock^);
                            try
                              AScript := ACache.Find(AItem.Text);
                              if not Assigned(AScript) then
                              begin
                                AScript := InternalCompile(AItem.Text, ScriptArray, False, Idle);
                                ACache.Add(AItem.Text, AScript);
                              end;
                            finally
                              Leave(ACache.Lock^);
                            end;
                          end
                          else AScript := InternalCompile(AItem.Text, ScriptArray, False, Idle);
                          AItem^ := MakeTextItem(Embrace(IntToStr(Add(ScriptArray, AScript)), BracketArray[btBrace]), FInternal.Handle^, -1);
                        finally
                          AScript := nil;
                        end;
                      end;
                ...
                try
                  if CFlag then Order(pcLocal, BItemArray, ScriptArray, Idle);
                  ...
                  begin
                    ...
                    begin
                      ...
                      case Data.TextType of
                        ttNumber:
                          begin
                            Check(Syntax, okNumber, AText, ScriptArray, -1);
                            Check(BItemArray, J, MakeItemData(Data.Value), ScriptArray, Parameter);
                            Write(icNumber);
                          end;
                        ttString:
                          if Parameter then
                            ...
                            raise Error(StringError, [Data.Text]);
                      else
                        if BItem.Text <> '' then raise Error(ElementError, [BItem.Text]);
                      end;
                    end
                    else if BItem.FHandle = FInternalHandle then
                    begin
                      AScript := GetIS(BItem.Text, ScriptArray, DFlag);
                      if Assigned(AScript) then
                      begin
                        if DFlag then
                          Check(Syntax, okParameter, AText, ScriptArray, -1)
                        else
                          Check(Syntax, okScript, AText, ScriptArray, -1);
                        Check(BItemArray, J, MakeItemData(AScript, DFlag), ScriptArray, Parameter);
                        if DFlag then
                          Write(icParameter)
                        else
                          Write(icScript);
                      ...
                      begin
                        ...
                        if S <> '' then raise Error(ElementError, [S]);
                      end;
                    end
                    else begin
                      Check(Syntax, okFunction, AText, ScriptArray, BItem.FHandle);
                      Check(BItemArray, J, MakeItemData(BItem.FHandle), ScriptArray, Parameter);
                      Write(icFunction);
                    end;
                  end;
                finally
                ...
                Check(Syntax, okLatest, AText, ScriptArray, -1);
              end;
              ...
              begin
                ...
                try
                  ...
                  try
                    if Sign then
                      BCache.Add(TOperatorArray[toNegative] + AItem.Text, AScript)
                    else
                      BCache.Add(AItem.Text, AScript);
                  finally
                    AScript := nil;
                  end;
                finally
...
procedure TParser.InternalOptimize(const Index: Integer; out AScript: TScript);
var
  ...
  case Item.Code of
    ...
    FunctionCode:
      begin
        J := Index;
        try
          if Optimizable(Index, AData.Number.Alive, Offset, Parameter, AScript) then
          begin
            ...
            if Assigned(AScript) then
            begin
              WriteScript(AData.ItemArray[I], AScript, Parameter);
              Inc(Index, Offset);
            end;
          end;
        finally
    ...
    ScriptCode:
      begin
        InternalOptimize(Integer(@Item.Script.Header), AScript);
        try
          AData.Number.Alive := Optimal(AScript, stScript);
          if AData.Number.Alive then
            AData.Number.Value := Execute(AScript)^
          else
            WriteScript(AData.ItemArray[I], AScript, False);
        finally
          AScript := nil;
        end;
        Inc(Index, SizeOf(TCode) + Item.Script.Header.ScriptSize);
...
function TParser.Order(const Coverage: TPriorityCoverage; var ItemArray: TTextItemArray; var ScriptArray: TScriptArray; var Idle: PFunction): Boolean;
          ...
          case AFunction.Priority.Priority of
              ...
              begin
                if AFunction.Method.Parameter.LParameter then
                  if (I - 1 = Low(ItemArray)) and ((ItemArray[I - 1].FHandle < 0) or (ItemArray[I - 1].FHandle = FInternalHandle)) then
                    ...
                    J := AFunction.Handle^;
                    S := Embrace(IntToStr(Add(ScriptArray, InternalCompile(Trim(S), ScriptArray, False, Idle))), BracketArray[btBrace]);
            ...
            try
              ...
              if Same(ItemArray, BItemArray) then Break
                ...
                S := Embrace(IntToStr(Add(ScriptArray, InternalCompile(S, ScriptArray, False, AFunction))), BracketArray[btBrace]);
...
begin
  ...
  case Item.Code of
    NumberCode:
      begin
        if not AData.Fake then
          I := Add(AData.ParameterArray^, ItemHeader.UserType.Handle, Item.ScriptNumber.Value);
    ...
    FunctionCode:
      begin
        if AData.Fake then
          ...
          I := Add(AData.ParameterArray^, ItemHeader.UserType.Handle, ExecuteFunction(AIndex, ItemHeader, EmptyValue));
      end;
    StringCode:
      begin
        if not AData.Fake then
        begin
          I := Add(AData.ParameterArray^, ItemHeader.UserType.Handle, '');
    ...
    ScriptCode:
      begin
        if not AData.Fake then
          I := Add(AData.ParameterArray^, ItemHeader.UserType.Handle, Execute(Integer(@Item.Script.Header))^);
  ...
  begin
    if (I >= 0) and Boolean(ItemHeader.Sign) then AData.ParameterArray^[I].Value := Negative(AData.ParameterArray^[I].Value);
    AData.Index^ := AIndex;
  end;
  Result := True;
end;
  ...
  case Item.Code of
    ...
    StringCode:
      begin
        ...
        WriteString(AData.ItemArray[I], Pointer(Index + J), Item.ScriptString.Size);
      ...
      begin
        ...
        try
          if Optimal(AScript, stScript) then
            WriteNumber(AData.ItemArray[I], Execute(AScript)^)
          else begin
            WriteScript(AData.ItemArray[I], AScript, False);
            AData.Optimal := False;
          end;
...
procedure TParser.Parse(var Text: string; var ScriptArray: TScriptArray; var Idle: PFunction);
var
  Data: TBracketData;
  I: Boolean;
begin
  FillChar(Data, SizeOf(TBracketData), 0);
  Data := MakeBracketData(@ScriptArray, False, Idle);
  Data.Text := TTextBuilder.Create;
  try
    ...
    begin
      ...
      if Data.Parameter then
        ParseBracket(Text, FParameterBracket, ParseMethod, @Data)
      else
        ParseBracket(Text, FBracket, ParseMethod, @Data);
    end;
  finally
    Data.Text.Free;
  end;
...
function TParser.ParseMethod(var Text: string; const StartIndex, EndIndex: Integer; const Data: Pointer): Boolean;
var
  S: string;
  AData: PBracketData absolute Data;
  I: Integer;
begin
  S := Trim(Copy(Text, StartIndex + 1, EndIndex - StartIndex - 1));
  if S = '' then raise Error(TextError, [Text]);
  I := Add(AData.ScriptArray^, InternalCompile(S, AData.ScriptArray^, AData.Parameter, AData.Idle));
  if AData.Parameter then
    S := ParameterPrefix
  else
    S := '';
  try
    AData.Text.Append(Copy(Text, 1, StartIndex - 1));
    AData.Text.Append(Embrace(S + IntToStr(I), BracketArray[btBrace]));
    AData.Text.Append(Copy(Text, EndIndex + 1, MaxInt));
...
procedure TParser.ReadParameterArray(var Index: Integer; out ParameterArray: TParameterArray;
  const ParameterType: TParameterType; const Fake: Boolean);
var
  Data: TParameterArrayData;
begin
  FillChar(Data, SizeOf(TParameterArrayData), 0);
  Data := MakeParameterArrayData(Fake, @ParameterArray, @Index);
...
begin
  ...
  begin
    ...
    Add(AData.ParameterArray^, ItemHeader.UserType.Handle, Value);
  end;
  case Item.Code of
    NumberCode:
      Inc(AIndex, SizeOf(TCode) + SizeOf(TScriptNumber));
...
procedure TParser.StringToScript(const Text: string; out AScript: TScript);
var
  AText: string;
  Idle: PFunction;
...
begin
  ...
  begin
    ...
    try
      AScript := FCache.Rawscript.Find(Text);
    finally
  ...
  begin
    ...
    Idle := nil;
    try
      AScript := InternalCompile(AText, ScriptArray, False, Idle);
    finally
      Delete(ScriptArray);
    end;
    if FCached and Assigned(FCache.Rawscript) then
    begin
      Enter(FCache.Rawscript.Lock^);
      try
        FCache.Rawscript.Add(Text, AScript);
      finally
        Leave(FCache.Rawscript.Lock^);
      end;
    end;
